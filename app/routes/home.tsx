...
import kirtiProfile from "./kirti-shah-profile.jpeg"; // Ensure this image exists in app/routes/
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-white antialiased selection:bg-[#2D3191]/10 selection:text-[#2D3191]">
      ...
      {/* 2. FOUNDER (Imported from Contact Us) */}
      <FounderSection />
      
      {/* 3. DESTINATIONS (Smart Grid with Modal) */}
      <DestinationsGrid onSelect={(d) => setSelectedDestination(d)} />
      ...
      {selectedDestination && (
        <DestinationModal 
          ...
          onPlan={() => handlePlanThis(selectedDestination.name)} 
        />
      )}
    </div>
  );
...
function DestinationModal({ destination, onClose, onPlan }: { destination: Destination, onClose: () => void, onPlan: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center px-4">
      ...
      <div className="relative bg-white rounded-3xl w-full max-w-4xl overflow-hidden shadow-2xl flex flex-col md:flex-row animate-active-up max-h-[90vh] md:max-h-auto overflow-y-auto md:overflow-visible">
        <button onClick={onClose} className="absolute top-4 right-4 z-10 w-10 h-10 bg-black/20 hover:bg-black/40 text-white rounded-full flex items-center justify-center backdrop-blur-md transition-colors"><X size={20} /></button>
        ...
        <div className="w-full md:w-1/2 p-8 flex flex-col justify-center">
          <span className="text-[#2D3191] font-bold text-xs uppercase tracking-widest mb-2">{destination.region} • {destination.category}</span>
          <h3 className="text-3xl font-bold text-[#1F2328] mb-4" style={{ fontFamily: "'Playfair Display', serif" }}>{destination.name}</h3>
          <p className="text-[#1F2328]/70 leading-relaxed mb-6">{destination.desc_short}</p>
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            <div className="bg-[#F7F6F1] p-3 rounded-xl border border-[#E6E8EF]">
              ...
              <p className="text-[#1F2328] font-medium">{destination.best_time}</p>
            </div>
            <div className="bg-[#F7F6F1] p-3 rounded-xl border border-[#E6E8EF]">
              <p className="text-xs text-[#1F2328]/50 uppercase font-bold">Ideal Duration</p>
              <p className="text-[#1F2328] font-medium">{destination.ideal_days}</p>
            </div>
          </div>

          <button onClick={onPlan} className="w-full py-4 bg-[#2D3191] text-white font-bold rounded-xl hover:bg-[#242875] transition-all flex items-center justify-center gap-2 shadow-lg hover:-translate-y-0.5">
            Get Quote for {destination.name} <ArrowRight size={18} />
          </button>
        </div>
      </div>
    </div>
