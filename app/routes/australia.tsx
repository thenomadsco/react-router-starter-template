...
const PAGE_CONFIG = {
  ...
  heroSlides: [
    { label: "Sydney Opera House", alt: "Sydney Opera House", src: "https://images.unsplash.com/photo-1506973035872-a4ec16b8e8d9?q=80&w=1920&auto=format&fit=crop" },
    { label: "Gold Coast", alt: "Surfers Paradise Skyline", src: "https://images.unsplash.com/photo-1533230985-7973d0947690?q=80&w=1920&auto=format&fit=crop" },
    { label: "Great Ocean Road", alt: "12 Apostles", src: "https://images.unsplash.com/photo-1510143110294-f442f4b00574?q=80&w=1920&auto=format&fit=crop" },
    { label: "Wildlife", alt: "Kangaroo in nature", src: "https://images.unsplash.com/photo-1528026732386-8a71569655d4?q=80&w=1920&auto=format&fit=crop" },
  ],

  itinerary: [
    {
      day: "01",
      title: "Arrival in Sydney – G'Day Mate!",
      desc: "Touch down at Sydney Kingsford Smith Airport. Transfer to your central hotel. In the evening, head to Darling Harbour, a lively waterfront precinct packed with restaurants and entertainment. Enjoy a relaxed dinner by the water as you settle into the Aussie vibe.",
    },
    {
      day: "02",
      title: "Sydney Icons & Bondi Beach",
      desc: "Start the day with a tour of the Sydney Opera House (outside) and walk across the Harbour Bridge for panoramic views. Later, we drive to the world-famous Bondi Beach. Walk the coastal path to Bronte or simply watch the surfers. In the afternoon, explore The Rocks district for some history and shopping.",
    },
    {
      day: "03",
      title: "Blue Mountains Day Trip",
      desc: "Escape the city for a day in the Blue Mountains. Visit Scenic World to ride the world's steepest railway and see the famous Three Sisters rock formation at Echo Point. On the way back, stop at a wildlife park to pet a kangaroo and take a photo with a koala. Return to Sydney in the evening.",
    },
    {
      day: "04",
      title: "Fly to Gold Coast – Surfers Paradise",
      desc: "Transfer to the airport for a short flight to the Gold Coast, Australia's holiday playground. Check into your hotel in Surfers Paradise. The rest of the day is free to enjoy the stunning long beach, visit the Skypoint Observation Deck for 360-degree views, or explore the Cavill Avenue shopping strip.",
    },
    {
      day: "05",
      title: "Movie World / Sea World",
      desc: "Today is for thrills! We include entry to one of the major theme parks. Choose Movie World to meet superheroes and ride rollercoasters, or Sea World to see dolphins, penguins, and polar bears. It’s a fun-filled day for the whole family.",
    },
    {
      day: "06",
      title: "Jet Boating & Hinterland",
      desc: "Start with an adrenaline-pumping Jet Boat ride through the Gold Coast waterways—expect 360-degree spins! In the afternoon, we take a quieter pace with a drive into the Gold Coast Hinterland. Visit Mount Tamborine for rainforest walks, waterfalls, and charming arts and crafts shops.",
    },
    {
      day: "07",
      title: "Departure",
      desc: "Enjoy a final breakfast by the beach. Depending on your flight, transfer to Brisbane or Gold Coast airport for your onward journey. You leave with memories of sun, surf, and unique wildlife.",
    },
  ],
};

// ... (Rest of component is standard boilerplate) ...
export function headers() { return { "Cache-Control": "no-store, no-cache, must-revalidate, proxy-revalidate" }; }
const iconDefaults = { size: 24, strokeWidth: 2 };
function IconBase({ size = iconDefaults.size, className, strokeWidth = iconDefaults.strokeWidth, fill = "none", children }: any) { return (<svg viewBox="0 0 24 24" width={size} height={size} className={className} fill={fill} stroke="currentColor" strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">{children}</svg>); }
...
export default function AustraliaPage() {
  ...
  useEffect(() => { const i = window.setInterval(() => { setActiveSlide((prev) => (prev + 1) % slides.length); }, 5000); return () => window.clearInterval(i); }, [slides.length]);
  return (
    <div className="min-h-screen bg-white antialiased selection:bg-[#2D3191]/10 selection:text-[#2D3191] pb-24 sm:pb-0">
      ...
      <section className="relative h-[85vh] min-h-[620px] flex items-end pb-20 px-6 sm:px-12 overflow-hidden">
        <div className="absolute inset-0 bg-[#2D3191]">
          {slides.map((slide, i) => (<img key={slide.src} src={slide.src} alt={slide.alt} className={["absolute inset-0 w-full h-full object-cover", "transition-[opacity,transform] duration-[1500ms]", "ease-[cubic-bezier(0.16,1,0.3,1)] will-change-transform", i === activeSlide ? "opacity-100 scale-[1.05]" : "opacity-0 scale-100"].join(" ")} loading={i === 0 ? "eager" : "lazy"} decoding="async" />))}
        </div>
        <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent" />
        <div className="relative z-10 max-w-[1400px] mx-auto w-full grid lg:grid-cols-2 gap-10 items-end">
          <div>
            <span className="px-4 py-2 bg-white/10 backdrop-blur-md border border-white/20 text-white text-xs font-bold uppercase tracking-widest rounded-full mb-6 inline-flex items-center gap-2">{PAGE_CONFIG.badge}<span className="w-1.5 h-1.5 rounded-full bg-[#02A551]" /><span className="text-white/90 font-semibold">{slides[activeSlide]?.label}</span></span>
          ...
          <div className="lg:text-right">
            <p className="text-white/80 text-lg mb-6">"Australia is just different. Come see why."</p>
      ...
      <section id="overview" className="py-20 px-6 sm:px-12 max-w-[1000px] mx-auto text-center">
        <h2 className="text-3xl font-semibold text-[#1F2328] mb-6" style={{ fontFamily: "'Playfair Display', serif" }}>City & Coast</h2>
        <p className="text-lg text-[#1F2328]/70 leading-relaxed">The best of Down Under in one week. Start with the iconic sights of Sydney—the Opera House, Harbour Bridge, and Bondi Beach—before flying to the Gold Coast for theme park thrills and endless surf beaches. It is the perfect Aussie starter pack.</p>
      </section>
      ...
      <section id="enquire" className="py-20 px-6 sm:px-12 max-w-[800px] mx-auto scroll-mt-24">
        <div className="bg-white p-8 sm:p-12 rounded-[2.5rem] border border-[#E6E8EF] shadow-xl text-center">
          <h2 className="text-3xl font-bold text-[#1F2328] mb-4">Dreaming of Australia?</h2>
          ...
          <form action="https://formsubmit.co/thenomadsco@gmail.com" method="POST" className="space-y-4 text-left">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1"><label className="text-sm font-semibold ml-1 text-[#1F2328]">Travelers</label><input type="text" name="travelers" className="w-full px-4 py-3 bg-[#FAFAF8] rounded-xl border border-[#E6E8EF] outline-none" placeholder="e.g. 2 Adults" /></div>
            </div>
