...
// --- DATA MOVED UP SO THE HERO CAN ACCESS IT ---
...
const CinematicHero = ({ onExplore }: { onExplore: () => void }) => {
  ...
  // Helper to ensure hero backgrounds are requested in high resolution (1920px wide)
  const getHighRes = (url: string) => url.replace("w=600", "w=1920").replace("w=3000", "w=1920");

  // Initialize with the first 3 international images (avoids a blank flash during load)
  const [backgrounds, setBackgrounds] = useState<string[]>([
    getHighRes(destinations[0].image),
    getHighRes(destinations[1].image),
    getHighRes(destinations[2].image)
  ]);

  useEffect(() => {
    // When the component mounts, filter for "International" and select 3 random images
    const intlImages = destinations
      .filter(d => d.category === "International")
      .map(d => getHighRes(d.image));
      
    // Shuffle the array and pick the top 3
