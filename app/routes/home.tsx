...
// --- DATASETS ---
// FIX: Use reliable Unsplash images for Santorini & Eiffel (Wikimedia FilePath can fail/CORS)
// (No placement changes — only the image sources.)
const heroImages = [
  {
  ...
  {
    url: "https://images.unsplash.com/photo-1511739001486-6bfe10ce7859?auto=format&fit=crop&w=2000&q=80",
...
// --- Styles from the previous file (text/panels/animations used there) ---
const customStyles = `
  ...
  const validate = (key: typeof current.key, raw: string) => {
    ...
    if (key === "bookingRef") {
      ...
      if (!/^[a-zA-Z0-9_-]{4,24}$/.test(v)) {
        return { ok: false as const, err: "Booking ref should be 4–24 chars (letters/numbers, optional - or _). Or type NA." };
      }
      return { ok: true as const, value: v };
    }
    ...
    if (key === "destination") {
      ...
      if (!/[a-zA-Z]/.test(v) || v.length < 2) return { ok: false as const, err: "Type a destination name (e.g., Dubai / Bali / Kashmir) 🙂" };
    ...
    if (key === "details") {
      ...
      if (/^(hi|hello|hey|test)$/i.test(cleaned)) return { ok: false as const, err: "Tell me what you want (enquiry/change/cancel details) 🙂" };
      return { ok: true as const, value: v };
    }
          ...
          {current?.key !== "done" ? (
            ...
            <div className="p-3 border-t border-black/10 space-y-2">
              ...
              <details className="text-xs text-gray-600">
                ...
                <pre className="whitespace-pre-wrap mt-2 bg-gray-50 p-2 rounded-xl border border-black/5">{whatsappText}</pre>
              </details>
            </div>
          )}
        </div>
...
// =================================================================================
// Sections copied/reverted to previous-file design where requested
// =================================================================================

// 2. FOUNDER (same text/colors as previous file; removed the 3-feature grid shown in your screenshot)
function FounderSectionPrevious_NoTrustGrid() {
  return (
    <section id="about" className="py-20 px-6 sm:px-12 bg-[#FAFAF8]">
      <div className="max-w-[1200px] mx-auto grid md:grid-cols-2 gap-16 items-center">
        ...
        <div>
          ...
          <h2
            className="text-4xl font-bold text-[#1F2328] mb-6"
            style={{ fontFamily: "'Playfair Display', serif" }}
          >
            Meet Kirti Shah
          </h2>
          <p className="text-lg text-[#1F2328]/70 leading-relaxed mb-6">
            Kirti believes that travel should be happy, not stressful. That's why she treats every client like
            family, personally overseeing every trip to ensure you are safe, comfortable, and having the time of
            your life.
          </p>
          <p className="text-lg text-[#1F2328]/70 leading-relaxed">
            With over 10 years of experience, we handle visas, flights, and bookings, offering luxury stays at
            best-value prices with 24/7 support.
          </p>

          {/* Removed the 3-feature area as requested (Visa & Flight Support / End-to-End Planning / Verified 4 & 5 Star Stays) */}
        </div>
      </div>
    </section>
  );
...
// 3. "Discover The Nomads" / Destinations trigger — exact style from previous file (white background)
function DestinationsTriggerPrevious({ onOpen }: { onOpen: () => void }) {
  return (
    <section
      id="destinations"
      className="py-24 px-6 bg-white flex flex-col items-center justify-center text-center"
    >
      <RevealOnScroll>
        ...
        >
          ...
          <div className="mt-8 flex justify-center opacity-0 transform translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500">
            <span className="flex items-center gap-2 text-lg font-medium text-[#2D3191]">
              Click to view all <ChevronDown className="w-5 h-5 -rotate-90" />
            </span>
          </div>
        </button>
      </RevealOnScroll>
...
// 5. ENQUIRY FORM — exact same as previous file (colors/panels/text)
function ContactSectionPrevious({ prefilledDestination }: { prefilledDestination: string }) {
  const inputClass =
    "w-full px-4 py-3 bg-[#FAFAF8] rounded-xl border border-[#E6E8EF] focus:ring-1 focus:ring-[#2D3191] outline-none text-[#1F2328] placeholder:text-gray-500";
      ...
      <div className="max-w-[1200px] mx-auto grid lg:grid-cols-12 gap-12 lg:gap-24">
        ...
        <div className="lg:col-span-5 space-y-8">
          ...
          <p className="text-lg text-[#1F2328]/70">
            We are ready to craft your perfect trip. Reach out to us directly or fill the form.
          </p>
    ...
    <div className="min-h-screen bg-white text-gray-900 font-sans overflow-x-hidden">
      ...
      <nav
        className={`fixed w-full z-50 transition-all duration-300 ${
          scrolled ? "bg-white/90 backdrop-blur-md py-4 shadow-sm border-b border-gray-100" : "bg-white/50 backdrop-blur-sm py-6"
        }`}
      >
        ...
        {isMenuOpen && (
          <div className="fixed inset-0 z-[100] bg-white flex flex-col pt-24 px-6">
            <button onClick={() => setIsMenuOpen(false)} className="absolute top-6 right-6 text-3xl font-light text-gray-900" aria-label="Close menu">
              ×
            </button>
      ...
      {/* Hero Section - edge-to-edge carousel (placement unchanged) */}
          ...
          <RevealOnScroll>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center space-y-4 sm:space-y-0 sm:space-x-6">
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                <button
                  onClick={() => scrollToSection("contact")}
                  className="px-10 py-4 bg-blue-600 text-white font-bold rounded-full hover:bg-blue-700 transition-all shadow-lg hover:shadow-xl hover:-translate-y-1 inline-flex items-center justify-center group"
                >
                  Start Planning <Calendar className="ml-2 w-5 h-5 group-hover:rotate-12 transition-transform" />
                </button>
                <a
                  href="tel:+919924399335"
                  aria-label="Call The Nomads Co"
                  className="px-10 py-4 bg-white/90 backdrop-blur-sm text-gray-900 font-bold rounded-full hover:bg-white transition-all shadow-lg hover:shadow-xl hover:-translate-y-1 inline-flex items-center justify-center border border-gray-200"
                >
      ...
      {/* Founder Section (reverted to previous) */}
      <FounderSectionPrevious_NoTrustGrid />
      ...
      {/* Destinations Trigger (reverted to previous, white background) */}
      <DestinationsTriggerPrevious onOpen={() => setShowDestinations(true)} />

      {/* Destinations Popup (unchanged) */}
      {showDestinations && (
      ...
      {/* Contact (reverted to previous exact form/panels/colors) */}
      <ContactSectionPrevious prefilledDestination={selectedDestination} />
      ...
      <footer className="relative z-10 bg-black text-white py-16">
        <div className="container mx-auto px-4 md:px-8">
          ...
          <div className="border-t border-gray-800 pt-8 flex flex-col md:flex-row justify-between items-center text-gray-400 font-medium">
            ...
            {/* Removed "Made with love in India" as requested */}
          </div>
        </div>
      </footer>
